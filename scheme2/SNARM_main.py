...
MIN_REPLAY_MEMORY_SIZE = 4_000  # Minimum number of steps in a memory to start training
...
class DQNAgent:
    ...
    def initilize_model(self):
        ...
        history = self.model.fit(self.normalize_data(train_data), train_label, epochs=20, validation_split=0.2,
                                 verbose=1)
    ...
    def deepDoubleQlearn(self, episode_done):
        ...
        next_Q_values_currentNetwork = self.model.predict(
            self.normalize_data(next_state_batch))  # use the current network to evaluate action
        next_actions = np.argmax(next_Q_values_currentNetwork, axis=1)

        next_Q_values = self.target_model.predict(
            self.normalize_data(next_state_batch))  # still use the target network to evaluate value
        ...
        self.model.fit(self.normalize_data(current_state_batch), Y, batch_size=MINIBATCH_SIZE, verbose=0, shuffle=False,
...
for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
    ...
    while not done:
        ...
        for temp_counter in range(sim_step_per_act_step):
            ...
            sim_next_state, sim_reward, sim_terminal, sim_outbound, sim_done = sim_env.simulated_step(sim_current_state,
                                                                                                      sim_action_idx,
                                                                                                      sim_cur_trajectory)
            episode_sim_reward += sim_reward
            sim_slide_window.append(
                (sim_current_state, sim_action_idx, sim_reward, sim_next_state, sim_terminal, sim_outbound, sim_done))
...
np.savez('MSE0',ep_MSE)
fig = plt.figure(60)
plt.plot(np.arange(len(ep_MSE)) + 1, ep_MSE, 'b-', linewidth=2)
plt.grid(which='major', axis='both')
plt.xlabel('Episode')
plt.ylabel('MSE')
fig.savefig('MSE0.eps')
fig.savefig('MSE0.pdf')
fig.savefig('MSE0.jpg')

np.savez('MAE0',ep_MAE)
fig = plt.figure(70)
plt.plot(np.arange(len(ep_MAE)) + 1, ep_MAE, 'b-', linewidth=2)
plt.grid(which='major', axis='both')
plt.xlabel('Episode')
plt.ylabel('MAE')
fig.savefig('MAE0.eps')
fig.savefig('MAE0.pdf')
fig.savefig('MAE0.jpg')

np.savez('Max_Absolute_Error0',ep_Max_Absolute_Error)
fig = plt.figure(80)
plt.plot(np.arange(len(ep_Max_Absolute_Error)) + 1, ep_Max_Absolute_Error, 'b-', linewidth=2)
plt.grid(which='major', axis='both')
plt.xlabel('Episode')
plt.ylabel('Maximum absolute eror')
fig.savefig('Max_Absolute_Error0.eps')
fig.savefig('Max_Absolute_Error0.pdf')
fig.savefig('Max_Absolute_Error0.jpg')

np.savez('Bin_cross_entr0',ep_bin_cross_entr)
fig = plt.figure(90)
plt.plot(np.arange(len(ep_bin_cross_entr)) + 1, ep_bin_cross_entr, 'b-', linewidth=2)
plt.grid(which='major', axis='both')
plt.xlabel('Episode')
plt.ylabel('Binary cross entropy')
fig.savefig('Bin_cross_entr0.eps')
fig.savefig('Bin_cross_entr0.pdf')
fig.savefig('Bin_cross_entr0.jpg')
...
# plt.ylim(-6000,0)
fig.savefig('return0.eps')
fig.savefig('return0.pdf')
fig.savefig('return0.jpg')
...
plt.show()
fig.savefig('CoverageMapLearned0.eps')
fig.savefig('CoverageMapLearned0.pdf')
fig.savefig('CoverageMapLearned0.jpg')
...
plt.ylabel('y (meter)', fontsize=14)
plt.show()
fig.savefig('trajectoriesSNARM0.eps')
fig.savefig('trajectoriesSNARM0.pdf')
fig.savefig('trajectoriesSNARM0.jpg')

print('{}/{} episodes reach terminal'.format(ep_reach_terminal.count(True), episode))

# Save the simulation ressult
np.savez('SNARM_main_Results0.npz', return_mov_avg, ep_rewards, ep_trajecotry)

...
@author: XGX
At each UAV location, get the empirical outage probability based on the measured signal
strengths
"""
...
SIR_THRESHOLD = -40  # SIR threshold in dB for outage

# ==========================================
# ==Simulate the building locations and building size.
# Each building is modeled by a square
...
Side = np.sqrt(A)  #建筑物的边长
#获得瑞利分布并以numpy数组形式返回随机样本
...
N_per_cluster = [np.ceil(N / Cluster) for i in range(Cluster)] #对元素向上取整,大于等于原来的值
...
for i in range(Cluster_per_side):
    for j in range(Cluster_per_side):
        ...
        X_loc = np.linspace((-Cluster_size + 2 * Side) / 2, (Cluster_size - 2 * Side) / 2,
                            Building_per_row)
...
step = 101
# include the start point at 0 and end point,
# the space between two sample points is D/(step-1)
HeightMapMatrix = np.zeros(shape=(D * step, D * step))  #大小
...
for i in range(N):
    ...
    y1 = YLOC[i] - Side / 2
    y2 = YLOC[i] + Side / 2
    HeightMapMatrix[int(np.ceil(x1 * step) - 1):int(np.floor(x2 * step)), #ceil,向上取整
    int(np.ceil(y1 * step) - 1):int(np.floor(y2 * step))] = H_vec[i]

# =================END of Building distributions================================

#==================random_generate_vehicle_position=======================
def random_generate_vehicle_position(num_states):
    ...
    return loc
# =============Define the vehicle Distribution=======================
BS_loc =random_generate_vehicle_position(7)
...
SectorBoreSiteAngle = [-120, 0, 120]  # the sector angle for each Vehicle
...
# ===========View Building and Vehicle distributions
plt.figure()
for i in range(N):
    x1 = XLOC[i] - Side / 2
    x2 = XLOC[i] + Side / 2
...
def getAntennaGain(Theta_deg, Phi_deg):
    ...
    AV = -np.min([12 * ((Theta_deg - 90) / angle3dB) ** 2, Am])
    # element power gain in Vertical
    Gemax = 8  # dBi antenna gain in dB above an isotropic radiator, Maximum directional
    # gain of an antenna element
    ...
    for n in range(ArrayElement_Horizontal):
        rMatrix[(n + 1) * np.arange(ArrayElement_Vertical), 2] = np.arange(
            ArrayElement_Vertical) * DV
    ...
    #逐元素返回复共轭
    ArrayFactor = np.conjugate(WeightFlatten).dot(SteeringVector.
                                    reshape(ArrayElement_Vertical, 1))
...
# =========Main Function that determines the best outage from all vehicle at a random location=======
# loc_vec: a matrix, nx3, each row is a (x,y,z) location
# SIR_th: the SIR threshold for determining outage,variable
...
# For a given location, return the empirical outage probability from all sectors of all Vehicles
# PointLoc:  the given point location
# SIR_th: the SIR threshold for defining the outage
# OutageMatrix: The average outage probability for connecting with each site,
# obtained by averaging over all the samples
...
# While the large scale path loss power is a constant for given location and site,
# the fast fading may change very fast.
# Hence, we return multiple fast fading coefficients. The number of samples is
# determined by FastFadingSampleSize
# A simple fast-fading implementation: if LoS, Rician fading with K factor 15 dB;
# otherwise, Rayleigh fading
def getReceivedPower_RicianAndRayleighFastFading(PointLoc, BS, LoS):
    ...
    if (PointLoc[1] == BS[1]) & (PointLoc[0] == BS[0]):
        ...
        Phi = np.arctan((PointLoc[1] - BS[1]) / (PointLoc[0] - BS[0] + 0.00001))
    ...
    RayleighComponent = np.sqrt(0.5) * (
                np.random.randn(FastFadingSampleSize, 3) +
                1j * np.random.randn(FastFadingSampleSize, 3))

    if LoS:  # LoS, fast fading is given by Rician fading with K factor 15 dB
        K_R_dB = 15  # Rician K factor in dB
        K_R = 10 ** (K_R_dB / 10)
        threeD_distance = 1000 * np.sqrt((BS[0] - PointLoc[0]) ** 2 + (BS[1] -
        PointLoc[1]) ** 2 + (BS[2] - PointLoc[2]) ** 2)  # 3D distance in meter
        DetermComponent = np.exp(-1j * 2 * np.pi * threeD_distance / WaveLength)
        # deterministic component
        AllFastFadingCoef = np.sqrt(K_R / (K_R + 1)) * DetermComponent + \
                            np.sqrt(1 / (K_R + 1)) * RayleighComponent
...
def checkLoS(PointLoc, BS):
    ...
    YRange = np.floor(YSample * (step - 1))  #向下取整
    XRange = [max(x, 0) for x in XRange]  # remove the negative idex
    YRange = [max(x, 0) for x in YRange]  # remove the negative idex
    #它是一个Python类型，代表一个标量NumPy类型,本质还是整形
    Idx_vec = np.int_((np.array(XRange) * D * step + np.array(YRange))) #怎么就成高度索引了?
    SelectedHeight = [HeighMapArray[0, i] for i in Idx_vec]
    #any，只要有一个true，就返回true；all，所有都为true。才返回true；
...
def getLargeScalePowerFromBS(PointLoc, BS, Theta_deg, Phi_deg, LoS):
    ...
    ChGain = np.power(10, ChGain_dB / 10)  # choose the sector that provides
    # the maximal channel gain
    Distance = 1000 * np.sqrt(
        (BS[0] - PointLoc[0]) ** 2 + (BS[1] - PointLoc[1]) ** 2 + (BS[2] - PointLoc[2]) ** 2)
    # convert to meter
    ...
    if LoS:
        ...
        PathLoss_NLoS_dB = -17.5 + (46 - 7 * np.log10(PointLoc[2] * 1000)) * np.log10(Distance) \
...
#    print(OutageMap[i,:])  用于调试的用途，后面可以注释掉
...
fig = plt.figure(10)
# plt.style.use('classic')
#画三维等高线图的，不同点在于contour() 是绘制轮廓线，contourf()会填充轮廓
...
# plt.show()
fig.savefig('CoverageMapTrue3.eps')
fig.savefig('CoverageMapTrue3.pdf')
fig.savefig('CoverageMapTrue3.jpg')

np.savez('radioenvir_-40dB', OutageMapActual, X_vec, Y_vec, TEST_LOC_meter, TEST_LOC_ACTUAL_OUTAGE)
